...
namespace Test.ExtendedServer.ExtendedServices
{
    public class ExtendedPriceService : PriceService
    {
        ...
        public override async Task<IList<PriceDetail>> EvaluatePricesAsync(int productId, string segment)
        {
            ...
            prices = [.. prices.Where(p => p.Segment == segment && p.From > DateTime.UtcNow && p.To < DateTime.UtcNow)];
